...
<head>
    ...
    <title>PocketMon - Mode Tamu | Pemasukan</title>
    ...
    <main class="flex-1 ml-64 overflow-y-auto">
        ...
        <div class="p-8">
            ...
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-semibold text-gray-800">Manajemen Pemasukan</h3>
                <button onclick="openModal('addModal')" class="flex items-center gap-2 px-5 py-2.5 bg-[#D1E7DD] hover:bg-green-200 text-green-700 font-semibold rounded-2xl transition text-sm">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse($incomes as $income)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-6 py-4 text-sm text-gray-600">{{ \Carbon\Carbon::parse($income['date'])->format('d M Y') }}</td>
                                <td class="px-6 py-4">
                                    <span class="px-3 py-1 bg-[#D1E7DD] text-green-700 text-xs font-medium rounded-full">{{ $income['category'] }}</span>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500">{{ $income['wallet_name'] ?? '-' }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500">{{ $income['description'] ?? '-' }}</td>
                                <td class="px-6 py-4 text-right text-sm font-semibold text-green-600">+Rp {{ number_format($income['amount'], 0, ',', '.') }}</td>
                                <td class="px-6 py-4 text-center">
                                    <form method="POST" action="{{ route('guest.incomes.destroy', $income['id']) }}" onsubmit="return confirm('Hapus pemasukan ini?')">
                                        @csrf @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-16 text-center">
                                    ...
                                    <p class="text-gray-400 text-sm">Belum ada pemasukan</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
...
<div id="addModal" class="fixed inset-0 bg-black/40 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl shadow-xl w-full max-w-md">
        <div class="p-6 border-b border-gray-100 flex items-center justify-between">
            <h3 class="font-semibold text-gray-800">Tambah Pemasukan</h3>
            <button onclick="closeModal('addModal')" class="text-gray-400 hover:text-gray-600"><i class="fa-solid fa-xmark"></i></button>
        </div>
        <form method="POST" action="{{ route('guest.incomes.store') }}" class="p-6 space-y-4">
            @csrf
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal</label>
                <input type="date" name="date" required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-green-300">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Nominal</label>
                <input type="number" name="amount" placeholder="0" required min="1" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-green-300">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Kategori</label>
                <select name="category" required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-green-300">
            ...
            <div>
                ...
                <select name="wallet_id" required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-green-300">
            ...
            <div>
                ...
                <textarea name="description" rows="2" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-green-300 resize-none"></textarea>
            </div>
            <button type="submit" class="w-full py-3 bg-[#D1E7DD] hover:bg-green-200 text-green-700 font-semibold rounded-2xl transition text-sm">Simpan</button>
        </form>
    </div>
</div>
